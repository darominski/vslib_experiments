...
TEST_F(ParameterRegistryTest, AutomaticParameterRegistration)
{
    ...
    const std::string registry_name = std::string("mockType") + '.' + std::string("mockName") + '.' + parameter_name;
...
TEST_F(ParameterRegistryTest, AutomaticMultipleParameterRegistration)
{
    ...
    EXPECT_NE(
        parameters.find(std::string("mockType") + '.' + std::string("mockName") + '.' + int_name), parameters.end()
    );
    EXPECT_NE(
        parameters.find(std::string("mockType") + '.' + std::string("mockName") + '.' + double_name), parameters.end()
    );
    EXPECT_NE(
        parameters.find(std::string("mockType") + '.' + std::string("mockName") + '.' + bool_name), parameters.end()
    );
    EXPECT_NE(
        parameters.find(std::string("mockType") + '.' + std::string("mockName") + '.' + string_name), parameters.end()
    );
    EXPECT_NE(
        parameters.find(std::string("mockType") + '.' + std::string("mockName") + '.' + enum_name), parameters.end()
    );
}
...
TEST_F(ParameterRegistryTest, NameExistsError)
{
    ...
    auto& registry = ParameterRegistry::instance();
    EXPECT_THROW(registry.addToRegistry("mockType.mockName.name", parameter), std::runtime_error);
}
