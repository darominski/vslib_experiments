...
namespace vslib
{
    ...
    void writeJsonToMessageQueue(
        const fgc4::utils::StaticJson& json_object, bmboot::MessageQueueWriter<SharedMemoryHeader>& message_queue
    )
    {
        auto serialized = json_object.dump();
        if (serialized.size() < fgc4::utils::constants::json_memory_pool_size)
        {
            SharedMemoryHeader             shared_memory(serialized.size());
    ...
    fgc4::utils::StaticJson readJsonFromMessageQueue(const std::pair<SharedMemoryHeader, std::span<uint8_t>>& message)
    {
