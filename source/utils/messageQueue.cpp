...
namespace fgc4::utils
{
    ...
    bool MessageQueueBase::writeMessage(void const* header, size_t header_size, std::span<uint8_t const> body)
    {
        ...
        //   +-----------+--------+------+
        //
        // The size of the header is NOT included; it is assumed that both the sender and receiver agree on its exact
        // structure (if they don't, knowing the size will not help you...) The header can also be void (0 bytes), as
        // can the body.
