...
namespace utils
{
    ...
    template<class BaseStates, class ExtraStates>
    class FsmTransitionResult
    {
      public:
        ...
        FsmTransitionResult(const std::variant<BaseStates, ExtraStates>& state, bool cascade = false)
            : m_state(state),
              m_default(false),
              m_cascade(cascade)
        {
        ...
        std::variant<BaseStates, ExtraStates> state() const
        {
            return m_state;
        }
      ...
      private:
        std::variant<BaseStates, ExtraStates> m_state;             //!< State of the transition
    ...
    template<class BaseStates, class ExtraStates, bool state_first = true>
    class Fsm
    {
      ...
      public:
        ...
        explicit Fsm(const BaseStates& starting_state)
        ...
        //! of the Parent class.
        void addState(
            const std::variant<BaseStates&, ExtraStates&> state, const StateFunc& state_func,
            const std::vector<TransitionFunc>& transitions
        )
        {
      ...
      private:
        ...
        struct StateObj
        {
            ...
            std::function<void(void)>   state_func;    //!< State function
