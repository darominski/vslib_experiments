...
auto prepareCommands(const std::vector<std::pair<std::string, std::string>>& parameters)
{
    ...
    commands.push_back({{"name", "brick_1.PID.pid_1.p"}, {"value", 0.2718}});
    return commands;
}

int main()
{
  std::cout << "Hello world from remote parameter setting!\n";
    ...
    // void* shared_memory = mmap(nullptr, total_memory_size, PROT_WRITE | PROT_READ, MAP_SHARED, shared_memory_field, 0);
